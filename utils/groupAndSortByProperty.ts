...
export function groupAndSortByProperty<T>(arr: T[], groupBy: keyof T, filterGroupBy?: keyof T): T[][] {
  ...
  for (let key in grouped) {
    const filterProp = !filterGroupBy ? groupBy : filterGroupBy
    grouped[key].sort((a, b) => String(a[filterProp]).localeCompare(String(b[filterProp])));
  }
